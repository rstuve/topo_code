...
from et_generator import makeList
...
else: # generate data in script
    for i in range(4):
        no_thresh_counts.append(len(makeList(i, start=start, stop=stop, threshold = False)))
        thresh_counts.append(len(makeList(i, start=start, stop=stop, threshold = True)))
        diffs.append(no_thresh_counts[i]-thresh_counts[i])
        percents.append(diffs[i]/no_thresh_counts[i]*100)
...
axs[0].legend(['with threshold', 'no threshold'])
