...
if __name__ == "__main__":
    ...
    for event in range(2):#NumOfEvents):
        showProgress(event)
        e = datatree[event]
        Eta = e['scells_eta']
        EtT = e['scells_Et']
        ...
        for Et_index in range(Ets_in_event):
            et = Etbin[Et_index]
            if et != None:
                if -EtaSet<=Eta[Et_index]<=EtaSet:
                    eta = round(Eta[Et_index], eta_dec)
                    if eta not in EtaList:
                        EtaList[eta] = []
                    EtaList[eta].append(et)
            else:
                excluded_etas += 1

    with open(SaveDirName+'eta_Et'+EXT, 'w+')as f:
        for eta in sorted(EtaList):
            f.write(str(eta)+': ')
            for et in EtaList[eta]:
                f.write(et+' ')
            f.write('\r\n')
